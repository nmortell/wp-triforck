...
//add replace on fileObj.filePath.replace("/wordpress/wp-admin/","") to workaround on windows server
	...
	test=file;
	var params = 'myajax=1&filename='+ uni_upload_dir+"/"+file.name + '&task=uni_resize_image_and_add&album_id='+jQuery('#multi_album_id').val();
...
function on_queue_completed(file)
{
	...
	uni_refresh_images_table(uni_current_album_id);
...
function uni_refresh_albums_table()
{
	...
	//sleep(2000);
	jQuery.post('index.php', 'task=uni_get_albums_table', function(res)
	{
		uni_set_data_tables();
		if (res.status == 'ok') 
		{
...
function uni_refresh_images_table(album_id)
{
	...
	jQuery.post('index.php', 'task=uni_get_albums_images_table&album_id='+album_id, function(res)
	{
		uni_set_data_tables();
		if (res.status == 'ok') 
		{
...
function re_order()
{
	...
	if( task == 'uni_reorder_album' )
		jQuery('#albums_table tbody').html('<tr><td colspan="7" align="center"><img src="'+loading.src+'" alt="" /><br/>Please wait, refreshing albums</td></tr>');
	if( task == 'uni_reorder_image' )
	...
	jQuery.get('index.php?' + params, function(res)
	{
		if( task == 'uni_reorder_image' )
		{
			uni_refresh_images_table(album_id);
		}
		if( task == 'uni_reorder_album' )
			uni_refresh_albums_table();
...
var tpager = null;
var ipager = null;
function uni_set_data_tables()
{
	//add event to bulk actions
	jQuery('.bulk_form').live('submit', uni_submit_bulk);
...
function uni_submit_bulk()
{
	var task = this.task.value;
	var album_id = null;
	if( task == 'uni_bulk_delete_albums' || task == 'uni_bulk_enable_albums' || task == 'uni_bulk_disable_albums')
	{
	...
	else if( task == 'uni_bulk_delete_images' || task == 'uni_bulk_enable_images' || task == 'uni_bulk_disable_images' )
	{
...
{
	//preload images
	loading.src = uni_url + '/images/loading.gif';
...
/*
		var ops = {
					'uploader'		: uni_url + '/js/uploadify-2.1.4/uploadify.swf',
					'script'		: uni_url + '/js/uploadify-2.1.4/uploadify.php',
					'cancelImg' 	: uni_url + '/js/uploadify-2.1.4/cancel.png',
				    'folder'    	: uni_upload_dir,
				    'auto'			: true,
					'multi'			: true,
					'fileExt'        : '*.jpg;*.gif;*.png',
...
*/
		var swfu;
			...
			var settings = {
				flash_url : uni_url+"/js/swfupload/js/swfupload.swf",
				upload_url: uni_url+"/js/swfupload/js/upload.php",
				post_params: {"PHPSESSID" : "<?php echo session_id(); ?>","folder":uni_upload_dir, "secw":uni_secw, nopar: "n" + new Date().getTime()},
				...
				// Button settings
				button_image_url: uni_url+"/js/swfupload/js/TestImageNoText_65x29.png",
				button_width: "130",
				button_height: "29",
	...
	jQuery('.uni_delete_album').live('click', uni_delete_album);
	...
	jQuery('table#album_images .enable, table#albums_table .enable').live('click', uni_enable);
	jQuery('table#album_images .disable, table#albums_table .disable').live('click', uni_disable);
